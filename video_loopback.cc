...
int RunLoopbackTest(int argc, char* argv[]) {
  ...
  absl::ParseCommandLine(argc, argv);
    ...
    // ---ylr begin
    absl::SetFlag(&FLAGS_force_fieldtrials, "WebRTC-VP8ConferenceTemporalLayers/3/WebRTC-VP8ScreenshareTemporalLayers/3/");
    
    absl::SetFlag(&FLAGS_duration, 4);
    absl::SetFlag(&FLAGS_num_temporal_layers, 0); //
    absl::SetFlag(&FLAGS_selected_tl, 0);
    absl::SetFlag(&FLAGS_num_spatial_layers, 0);
    absl::SetFlag(&FLAGS_selected_sl,0);
    absl::SetFlag(&FLAGS_num_streams, 2);
    absl::SetFlag(&FLAGS_selected_stream, 0);
    absl::SetFlag(&FLAGS_send_side_bwe, true);
    absl::SetFlag(&FLAGS_generic_descriptor, false);
    absl::SetFlag(&FLAGS_allow_reordering, false);
    absl::SetFlag(&FLAGS_use_ulpfec, false);
    absl::SetFlag(&FLAGS_use_flexfec, false);
    absl::SetFlag(&FLAGS_audio, false);
    absl::SetFlag(&FLAGS_audio_video_sync, false);
    absl::SetFlag(&FLAGS_video,true);
    absl::SetFlag(&FLAGS_logs, true);
    
    absl::SetFlag(&FLAGS_codec, "VP8");
    absl::SetFlag(&FLAGS_width, 640);
    absl::SetFlag(&FLAGS_height, 480);
    absl::SetFlag(&FLAGS_fps, 15);
    absl::SetFlag(&FLAGS_suspend_below_min_bitrate, false);
    absl::SetFlag(&FLAGS_inter_layer_pred, 2);
    
    
    absl::SetFlag(&FLAGS_loss_percent, 0);
    absl::SetFlag(&FLAGS_avg_burst_loss_length, -1);
    absl::SetFlag(&FLAGS_link_capacity, 0);
    absl::SetFlag(&FLAGS_queue_size,0);
    absl::SetFlag(&FLAGS_avg_propagation_delay_ms,0);
    absl::SetFlag(&FLAGS_std_propagation_delay_ms,0);
    absl::SetFlag(&FLAGS_min_bitrate, 50);
    absl::SetFlag(&FLAGS_start_bitrate, 300);
    absl::SetFlag(&FLAGS_target_bitrate, 800);
    absl::SetFlag(&FLAGS_max_bitrate, 800);
    // ---ylr end
